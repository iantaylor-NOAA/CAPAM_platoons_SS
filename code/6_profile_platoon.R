...
for (icase in cases) {
  ...
  if (icase %in% grep("A_IIa_Fp4", cases, value = TRUE)) {
    ...
    build_models(
      ...
      cwe = cwe, M_val = 0.1, CV_vals = c(0.1, 0.1)
    ) # based on what we were told the setting in the IBM was...
  }
  if (icase %in% grep("B_IIa_Fp4", cases, value = TRUE)) {
    ...
    build_models(
      run = 1:n, updatedat = TRUE, dir = mydir_today_plat,
      use_initF = TRUE, dir.template = dir.template_current, agelen = agelen,
      cwe = cwe, M_val = 0.1, CV_vals = c(0.2, 0.2)
    ) # based on what we were told the setting in the IBM was...
  }
  if (icase %in% grep("A_IIa_1WayTrip", cases, value = TRUE)) {
    build_models(
      ...
      agelen = agelen,
      cwe = cwe, M_val = 0.1, CV_vals = c(0.1, 0.1)
    ) # base on what we were told the setting in the IBM was
  }
...
for (icase in cases) {
  ...
  dat <- r4ss::SS_readdat(file.path(
    plat_scen_path, "CAPAM_platoons_run001",
    "platoons_data.ss"
  ))
  lapply(vec_of_plat_ratio, function(ratio, scen_path, dat, ss_name) {
    ...
    r4ss::copy_SS_inputs(
      dir.old = file.path(scen_path, "CAPAM_platoons_run001"),
    ...
    r4ss::SS_writectl(ctl,
      outfile = file.path(dir_new_ratio, "platoons_control.ss"),
      overwrite = TRUE
    )
    # Run the model
    SSutils::run_SS_models(
      dirvec = dir_new_ratio,
      systemcmd = TRUE, skipfinished = FALSE,
      extras = "-nox", model = ss_name, exe_in_path = TRUE,
      intern = TRUE
    )
